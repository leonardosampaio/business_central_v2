...
$payment_method_id = 'payment_method_id_example'; // string | (v1.0) id for paymentMethod
 ...
 **payment_method_id** | **string**| (v1.0) id for paymentMethod |
...
```php
getPaymentMethod($company_id, $payment_method_id, $select): \OpenAPI\Client\Model\PaymentMethod
```
...
try {
    $result = $apiInstance->getPaymentMethod($company_id, $payment_method_id, $select);
    print_r($result);
} catch (Exception $e) {
...
```php
listPaymentMethods($company_id, $top, $skip, $limit, $filter, $select): \OpenAPI\Client\Model\InlineResponse20022
```
...
try {
    $result = $apiInstance->listPaymentMethods($company_id, $top, $skip, $limit, $filter, $select);
    print_r($result);
} catch (Exception $e) {
...
```php
patchPaymentMethod($company_id, $payment_method_id, $content_type, $if_match, $unknown_base_type): \OpenAPI\Client\Model\PaymentMethod
```
...
try {
    $result = $apiInstance->patchPaymentMethod($company_id, $payment_method_id, $content_type, $if_match, $unknown_base_type);
    print_r($result);
} catch (Exception $e) {
...
```php
postPaymentMethod($company_id, $content_type, $unknown_base_type): \OpenAPI\Client\Model\PaymentMethod
```
...
try {
    $result = $apiInstance->postPaymentMethod($company_id, $content_type, $unknown_base_type);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PaymentMethodApi->postPaymentMethod: ', $e->getMessage(), PHP_EOL;
}
```
